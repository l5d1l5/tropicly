...
for(i in 1:4) {
  for(j in 1:4) {
    equal[j, i] = wilcox.test(americas[,i], americas[,j], paired = TRUE, alternative = 'two.sided')$p.value
    greater[j, i] = wilcox.test(americas[,i], americas[,j], paired = TRUE, alternative = 'greater')$p.value
    less[j, i] = wilcox.test(americas[,i], americas[,j], paired = TRUE, alternative = 'less')$p.value
  }
}
