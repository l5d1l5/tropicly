...
%TODO flowchart of all processes
	...
	\subsection{Deforestation} 
		...
		%TODO add wilcoxon rank sum test for region comparison
		%TODO Holm correction to text
		% Further we applied a wilcoxon rank sum test to examine the similarity between regions also known as  Mann–Whitney U test. To examine regional behavior we grouped by regions and applied test, to determine the best canopy density class for our study we applied test over the entire study extent. To address the higher probability of family-wise error in multiple comparisons we used a holm correction. 
		...
		%TODO table: convert expressions to i,j
	...
	%TODO add  2007 Int'I\$ y^-1 ha^-1, Geary-Khamis Dollar
		...
		\end{equation}
		We aggregated our results for \ac{ESV} loss, \ac{ESV} gain, and \ac{ESV} balance on three spatially different levels with the following extent: global tropical zone and continental region (Americas, Asia, and Africa).

	\subsection{Binning analysis and visualization}
	%TODO image: hexagon countries
	%TODO hexagonal binning image: needs D, change y_2 to y_1, change equations to explain
