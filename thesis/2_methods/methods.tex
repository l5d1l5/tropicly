...
%TODO flowchart of all processes
	...
	\subsection{Deforestation} 
		...
		%TODO add blue line to aism image
		%TODO add wilcoxon rank sum test for region comparison and write we compared for all regions
		%TODO further we applied a wilcoxon rank sum test to examine the similarity between regions also known as  Mann–Whitney U test
		%TODO to examine regional behavior we grouped by regions and applied test, to determine the best canopy density class for our study we applied test over the entire study extent
		...
		%TODO visual interpretation what are the rules u used
		%TODO decision between regrowth and primary, natural forest based on detection of experct forest worker
		%TODO plantend forests are age class so they have a homogeneous canopy
		%TODO reference olofsson kappa coefficient is discouraged but for the sake of completness we compute it
	...
	\label{subsec:esv_methods}
		...
		\end{equation}
		We aggregated our results for \ac{ESV} loss, \ac{ESV} gain, and \ac{ESV} balance on three spatially different levels with the following extent: global tropical zone, per continental region (Americas, Asia, and Africa), and per tropical country.
	...
	%TODO image change equations to explained
