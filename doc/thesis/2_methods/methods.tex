...
%TODO put in each dataset section why you use the data as first paragraph/sentence. SO WHAT??? 
	Table \ref{tab:datasets} shows a comprehensive overview of the applied datasets for this study. Spatial datasets comprises vector as well raster data and empirical data is extracted from the cited publications. Each dataset is introduced in a extra section following the order of appearance in the table.
	\begin{table}[ht]
		\centering
		\caption[Datasets used during this study]{\textbf{Datasets used during this study:} The source column contains the reference to the corresponding release publication. If the data is provided as a download the reference contains the corresponding download URL.}
		\label{tab:datasets}
		\begin{tabular}{lll}
			\hline
...
%TODO flowchart of all processes
	...
	\subsection{Deforestation}
		...
		%TODO set number of tiles
			...
			Now we have to check if the tree cover similarity increases or decreases, hypothesis is the similarity of GL30 and GFC tree cover is max when the selected canopy density is 10-100 in case of tropical africa or 30-100 percent in tropical asia and america. We use inferential statistics and cross testing to check our assumptions. We grouped the computed jaccard indexes per continent/region as the red line in figure 1.3 shows. To avoid to deduce the underlying distribution of the jaccard indexes we used a nonparametric test called wilcoxon signed rank test. this test requires in minimum ordinal scaled samples, the dependent variable is continues and each pair is independent. This criteria are meet by our dataset. We applied the wilcoxon test as two and one sided. The test procedure is implemented by means of R.

		\subsubsection{Proximate deforestation driver}
		%TODO flowchart needs reclassification
		%TODO cite hoshen kopelman
